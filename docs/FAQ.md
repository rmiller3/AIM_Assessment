1.  Elevator Counts are Undefined:  <p>
...
```A person requests an elevator be sent to their current floor```<p>
...
2.  Elevator "Areas" are Undefined: <p>
You can imagine a large building with multiple 'wings'.  Each 'wing' might have a separate elevator system, with separate request buttons that only summon an elevator for that area.  For example, pressing the button in the East Wing should not summon an elevator to that floor in the West Wing.  <p>
...
3.  What happens when a user enters an elevator going up, then boards the elevator and presses a floor below them.<p>
...
4.  When is the floor request button reset?  <p>
For example, a person on the 4th floor requests to go up.  He enters the elevator, but has not yet pressed the destination button within the elevator.  The elevator doors close or start closing.  As that happens, another user approaches the 4th floor wanting to go up.<p>
...
5.  Do we need to support express elevators?<p>
...
6.  Do we need to support capacity / weight restrictions on elevators?<p>
...
7.  Next Steps?<p>
    1.  Write the methods to simulate the elevator visiting a floor and removing users from the queue.
